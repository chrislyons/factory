#!/usr/bin/env python3
"""
Benchmark Gemma4 E2B with KV cache quantization.
Tests whether KV quant helps speed on small dense models (memory-bandwidth-bound).
"""
...
MODEL_PATH = "/Users/nesbitt/models/gemma-4-e2b-6bit"
...
print(f"Layers: {len(pc)} total, {n_kv} KV cache")

# KV cache per token (fp16)
# Gemma4 E2B: kv_heads=1, head_dim=256
kv_bytes_per_token_fp16 = n_kv * 2 * 1 * 256 * 2
print(f"KV per token fp16: {kv_bytes_per_token_fp16} bytes = {kv_bytes_per_token_fp16/1024:.1f} KB")
...
# Summary
print(f"\n{'='*70}")
print("SUMMARY — Gemma4 E2B KV Cache Quantization")
print(f"{'='*70}")
...
with open("/tmp/e2b_kv_benchmark.json", "w") as f:
    json.dump(results, f, indent=2)
print("Results saved to /tmp/e2b_kv_benchmark.json")
