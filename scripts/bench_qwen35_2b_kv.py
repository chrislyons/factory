#!/usr/bin/env python3
"""Benchmark Qwen3.5-2B with KV cache quantization."""
...
MODEL_PATH = "/Users/nesbitt/models/Qwen3.5-2B-6bit"
...
# KV heads and head_dim from config
# qwen3_5: kv_heads=2, head_dim=256, 24 layers
# But this is a conditional generation model - check text_config
import json as json_mod
with open(f"{MODEL_PATH}/config.json") as f:
    cfg = json_mod.load(f)
tc = cfg.get("text_config", {})
kv_heads = tc.get("num_key_value_heads", cfg.get("num_key_value_heads", 2))
...
print(f"\n{'='*70}")
print("SUMMARY — Qwen3.5-2B KV Cache Quantization")
print(f"{'='*70}")
...
with open("/tmp/qwen35_2b_kv_benchmark.json", "w") as f:
    json.dump(results, f, indent=2)
print("Results saved to /tmp/qwen35_2b_kv_benchmark.json")
