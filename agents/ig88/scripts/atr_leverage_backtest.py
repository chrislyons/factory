#!/usr/bin/env python3
"""
ATR Breakout Backtest with Leverage - Walk-Forward Validation
Tests Donchian(20) breakout with ATR(10) stop across 6 assets, 3 leverage levels, 4 splits.
"""
...
OUTPUT_FILE = Path('/Users/nesbitt/dev/factory/agents/ig88/data/atr_leverage_validation.json')

SYMBOLS = ['AVAX', 'ETH', 'LINK', 'NEAR', 'SOL', 'BTC']
...
def load_data(symbol: str) -> pd.DataFrame:
    ...
    if len(df) < 43788:
        raise ValueError(f"Insufficient data for {symbol}: {len(df)} rows (need 43788+)")
...
def run_backtest(df: pd.DataFrame, leverage: int) -> dict:
...
def calculate_metrics(trades: list, df: dict) -> dict:
...
def main():
    print("=" * 80)
    print("ATR BREAKOUT BACKTEST - LEVERAGE VALIDATION")
    print("=" * 80)
        ...
        try:
            ...
            for leverage in LEVERAGE_LEVELS:
                ...
                for split_pct in SPLIT_PCTS:
                    ...
                    metrics = calculate_metrics(trades, test_df)
