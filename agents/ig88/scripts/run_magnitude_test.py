...
class HighMagnitudeSurgicalTester:
    ...
    def compute_indicators(self, df: pd.DataFrame):
        ...
        return {
            "c": c, "h": h, "l": l, "atr": atr_v, "rsi": rsi_v, 
            "cloud_top": cloud_top, "b_pct": b_pct, "ma20": ma20, 
            "atr_baseline": atr_baseline
        }

    def run_test(self, asset: str, mode: str, friction: float = 0.01):
        df = self.data[asset]
        ...
        for i in range(50, len(c)-1):
            ...
            if mode == "VOL_GATED_TREND":
                if (atr_v[i] > p['atr_baseline'][i] * 1.2) and (c[i] > p['cloud_top'][i]) and (p['rsi'][i] > 55):
                    entry_price = c[i]
            
            elif mode == "EXTREME_REV":
                if (p['b_pct'][i] < 0.05) and (p['rsi'][i] < 25):
                    entry_price = c[i]
            ...
            if entry_price is not None:
                ...
                is_trailing = (mode == "VOL_GATED_TREND")
                exit_price = c[-1]
                ...
                if is_trailing:
                    ...
                    target = entry_price + 3.0 * atr_v[i]
                    for j in range(i+1, len(c)):
                        if c[j] <= stop or c[j] >= target:
                            exit_price = c[j]
                            break
        ...
        return {"trades": total_trades, "win_rate": (total_wins/total_trades*100 if total_trades>0 else 0), "pf": pf, "net_pnl": (gross_wins - gross_losses)*100}

if __name__ == "__main__":
    assets = ["SOL/USDT", "BTC/USDT", "ETH/USDT", "LINK/USDT", "AVAX/USDT", "NEAR/USDT"]
    tester = HighMagnitudeSurgicalTester(assets, Path("/Users/nesbitt/dev/factory/agents/ig88/data"))
    
    modes = ["VOL_GATED_TREND", "EXTREME_REV"]
    results = {}
    
    for asset in assets:
        asset_res = {}
        for mode in modes:
            asset_res[mode] = tester.run_test(asset, mode, friction=0.01)
