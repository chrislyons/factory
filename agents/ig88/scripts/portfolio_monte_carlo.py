...
def simulate_asset_annual_returns(stats, n_iter=ITERATIONS, seed=None):
    """Simulate annual returns for a single asset using trade return distribution."""
    if seed is not None:
        np.random.seed(seed)
...
def simulate_portfolio(alloc, n_iter=ITERATIONS):
    ...
    # Combine asset annual returns according to weights
    # Assume independence between assets (correlation not modeled)
    portfolio_returns = np.zeros(n_iter)
    for asset, weight in alloc.items():
        if asset in asset_annual_returns:
            # sample n_iter returns from asset's simulated returns
            # we already have n_iter simulated returns per asset, just take them
...
def portfolio_results(returns):
    ...
    # Max drawdown: compute across simulated years? We only have annual returns, not path.
    # Approximate max drawdown as the worst peak-to-trough within the year? Not possible.
    # Instead compute the worst annual return (single year drawdown).
    # We'll compute the 5th percentile as worst case.
    # Sharpe ratio: assume risk-free rate = 0 (crypto)
    sharpe = np.mean(returns) / np.std(returns) if np.std(returns) > 0 else 0
    # Max drawdown: simulate a random sequence of 5 years of returns and compute max drawdown.
    # We'll do a simple approximation: assume independence across years, simulate 5-year paths.
    n_years = 5
    n_paths = 1000
    max_dd_samples = []
...
output = {
    "metadata": {
        ...
        "notes": "Monte Carlo simulation using trade return distributions from walk-forward validation. Assumes CV=0.5 for win/loss distributions, independence across assets, Poisson-distributed trade counts.",
    },
    "results": results,
}
