    ...
    >
      ...
      >
        {pendingItems.length === 0 ? (
          <EmptyState title="No pending approvals" detail={pending.error ? "Waiting for coordinator…" : "The approval queue is clear."} />
        ) : (
      ...
      >
        {showResolved ? (
          resolvedItems.length === 0 ? (
            <EmptyState title="No resolved approvals yet" detail="Coordinator history will appear here." />
          ) : (
          ...
          <div className="placeholder-copy">Collapsed to keep the pending queue dense and operator-friendly.</div>
        )}
      </SurfaceCard>
    </AppShell>
  );
